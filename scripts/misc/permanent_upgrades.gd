...
const CARD_BG := Color(0.18, 0.16, 0.28, 0.95)
...
func _ready() -> void:
	upgrades_grid.columns = GRID_COLUMNS
...
func _refresh_ui() -> void:
	...
	for upgrade_id in PlayerData.UPGRADE_IDS:
		upgrades_grid.add_child(_build_upgrade_card(upgrade_id))


func _build_upgrade_card(upgrade_id: String) -> PanelContainer:
	var def: Dictionary = PlayerData.UPGRADE_DEFS[upgrade_id]
	var level := PlayerData.get_upgrade_level(upgrade_id)
	var max_level: int = PlayerData.get_max_upgrade_level(upgrade_id)
	...
	panel.custom_minimum_size = Vector2(280, 148)
	panel.tooltip_text = def["description"]
	panel.add_theme_stylebox_override("panel", _panel_style(CARD_BG))
	...
	if level >= max_level:
		bonus.text = "%s (maxed)" % PlayerData.format_upgrade_bonus(upgrade_id, current_bonus)
		bonus.add_theme_color_override("font_color", MAXED_COLOR)
	else:
		var next_bonus: float = float(def["bonuses"][level])
		bonus.text = "%s\nNext: %s" % [
			PlayerData.format_upgrade_bonus(upgrade_id, current_bonus),
			PlayerData.format_upgrade_bonus(upgrade_id, next_bonus, true),
		]
